(function () {
  ...
  let spatialAnnotationsLoading = null;

  function spatialAnnotationsScriptUrl() {
    const configured = document.querySelector(
      'script[src*="spatial_annotations.js"]',
    );
    if (configured && configured.src) {
      return configured.src;
    }
    const runtime = document.querySelector(
      'script[src*="workstream_5w2h.js"]',
    );
    if (!runtime || !runtime.src) {
      throw new Error(
        "Não foi possível localizar o módulo de anotações do InfoBIM.",
      );
    }
    return runtime.src.replace(
      /workstream_5w2h\.js(?:\?.*)?$/,
      "spatial_annotations.js?v=20260730-spatial-annotations-13",
    );
  }

  async function ensureSpatialAnnotations() {
    if (window.InfoBIMSpatialAnnotations) {
      return window.InfoBIMSpatialAnnotations;
    }
    if (spatialAnnotationsLoading) {
      return spatialAnnotationsLoading;
    }
    spatialAnnotationsLoading = new Promise((resolve, reject) => {
      const script = document.createElement("script");
      script.src = spatialAnnotationsScriptUrl();
      script.async = true;
      script.addEventListener("load", () => {
        if (window.InfoBIMSpatialAnnotations) {
          resolve(window.InfoBIMSpatialAnnotations);
          return;
        }
        reject(new Error(
          "O módulo de anotações foi carregado, mas não foi inicializado.",
        ));
      }, { once: true });
      script.addEventListener("error", () => {
        reject(new Error(
          "Não foi possível carregar spatial_annotations.js do projeto.",
        ));
      }, { once: true });
      document.head.appendChild(script);
    });
    try {
      return await spatialAnnotationsLoading;
    } catch (error) {
      spatialAnnotationsLoading = null;
      throw error;
    }
  }
  ...
  function clearPreview(row, message = "Selecione um arquivo.") {
    ...
    if (window.InfoBIMSpatialAnnotations) {
      window.InfoBIMSpatialAnnotations.clearPreview({
